document.querySelectorAll("[data-numspin").forEach((function (t) {
    ...
    n.className = "input-group input-numspin-element", e.replaceChild(n, t), n.appendChild(t);
    t.insertAdjacentHTML("beforebegin", '<button class="border border-0 bg-white minus-btn" type="button">-</button>');
    var s, a;
    t.insertAdjacentHTML("afterend", '<button class="border border-0 bg-white plus-btn" type="button">+</button>');
    var i = t;
    ...
    (new DOMParser).parseFromString(n, "text/html"), n.querySelector(".plus-btn").addEventListener("click", (function () {
        newInputVal = parseInt(i.value, 10) + step, newInputVal <= s && (i.value = newInputVal)
    })), n.querySelector(".minus-btn").addEventListener("click", (function () {
        newInputVal = parseInt(i.value, 10) - step, newInputVal >= a && (i.value = newInputVal)
    }))
}));